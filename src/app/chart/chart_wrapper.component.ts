...
import * as datasets from "./datasets.json"; // From data gathered using the FamDB/utils/famdb_pie_stats.py script
...
export class ChartWrapperComponent implements AfterViewInit {
    ...
    drawChart(): void {
        ...
        // Compute per-side label positions, nudging labels apart when they would overlap.
        const labelPos: Record<string, [number, number]> = {};
        const minSpacing = 14; // px — matches the 11px font with a small gap
        const pieData = pie(this.data);
        const radius = this.radius;

        const resolveCollisions = (labels: any[], isRight: boolean) => {
            labels.sort((a, b) => outerArc.centroid(a)[1] - outerArc.centroid(b)[1]);
            let prevY = -Infinity;
            for (const d of labels) {
                const y = Math.max(outerArc.centroid(d)[1], prevY + minSpacing);
                prevY = y;
                labelPos[d.data.group] = [radius * (isRight ? 1 : -1), y];
            }
        };

        resolveCollisions(pieData.filter(d => midAngle(d) < Math.PI), true);
        resolveCollisions(pieData.filter(d => midAngle(d) >= Math.PI), false);
